...
public sealed class SpaceSnoopTools
{
    ...
    [Description("Сканирует каталог или диск и возвращает распределение занятого места: крупнейшие подкаталоги до заданной глубины и крупнейшие файлы всего дерева. Только чтение – ничего не удаляет. При show=true результат попадает в дерево страницы «Сканирование», и после этого по нему работают mark_for_deletion и archive_directory; без show это отдельный расчёт, окно о нём не знает. Движков обхода два, применённый лежит в options.engine: «directories» ходит по каталогам и считает жёсткую ссылку под каждым именем, «mft» читает таблицу NTFS и считает файл один раз (сколько на этом снято двойного счёта – notes.extraNameBytes). Итог бывает неполным, и молчать об этом нельзя: notes.droppedObjects и notes.droppedBytes – то, чего в дереве нет вовсе, notes.partialRecords – записи, оставшиеся без части данных; при непустых notes оговаривайте это в ответе, а числа notes у движка «mft» относятся ко всему тому, даже когда сканировали подкаталог.")]
